...
まとめたアプリは **幅調整（✏ 分割比率のドラッグ）**・**ウィンドウのリサイズ（分割比率を維持）**・**グリッドサイズ変更（2×2 ⇄ 1×4 など）**・**Retile（↺ 再配置）** で自在にレイアウトできます。

## 🎨 大量のターミナルを色で一望

<p align="center">
  <img src="assets/demo-terminals.gif" alt="大量の縦長ターミナルを横一列に整理し状態色で一望" width="100%">
  <br><sub><i>※ 動作イメージ（UI を再現したアニメーション）</i></sub>
</p>

縦長のターミナルを横一列に整列し、状態を色で表示（🔵 実行中 / 🟢 完了 / 🟡 入力待ち / 🔴 要対応）。大量の並列タスクでも、ヘッダーの集計と合わせて一目で「どれに対応すべきか」判断できます。
...
最新版を [Releases](https://github.com/lutelute/TerminalIN/releases/latest) から取得できます。
...
<br><sub><b>macOS 15 (Sequoia) 以降</b></sub>
<br><code>*-arm64-mac.zip</code> を解凍 →
<br><code>TiN.app</code> を <code>/Applications</code> へ
</td>
<td align="center" width="50%">
...
<br><sub><b>Windows 10 / 11</b></sub>
<br><code>TiN-Setup-*.exe</code>（インストーラ）
<br>または <code>*-win.zip</code>（ポータブル）
</td>
</tr>
</table>

> 🍎 **macOS**: 初回起動時に **Accessibility 権限**（システム設定 > プライバシーとセキュリティ > アクセシビリティ）を許可してください。
> 🪟 **Windows**: 未署名のため SmartScreen が出たら「詳細情報」→「実行」。
...
TiN は **macOS（作者環境）** と **Windows 11 arm64（Parallels 実機）** で動作確認済みですが、まだ確認できていない環境が多くあります。以下の環境での動作報告を募集しています:

| 環境 | 状況 |
|------|------|
| <img src="assets/logo-apple.svg" height="12"> Apple Silicon (M1〜M4) + macOS 26 | ✅ 作者環境で確認済み |
| <img src="assets/logo-apple.svg" height="12"> Apple Silicon + macOS 15 (Sequoia) | ❓ 未確認 — 報告求む |
| <img src="assets/logo-apple.svg" height="12"> Intel Mac | ❓ 未確認 — 報告求む（ビルドターゲットは arm64 のみのため `package.json` の調整が必要かも） |
| <img src="assets/logo-windows.svg" height="11"> Windows 11 arm64 | ✅ Parallels 実機で起動・内蔵端末・native ロード確認済み |
| <img src="assets/logo-windows.svg" height="11"> Windows 10 / 11 x64 | ❓ CI ビルドのみ — 実機報告求む |
| <img src="assets/logo-windows.svg" height="11"> Windows での外部ウィンドウ snap | ❓ API ロードまで確認、実運用報告求む |
| iTerm2 / Warp / Alacritty での snap | ❓ 部分的にしか確認していません |
| マルチディスプレイ構成 | ❓ 2 枚までしか確認していません |
