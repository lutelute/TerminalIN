...
<p align="center">
  <b>macOS 向け Groupy スタイルのウィンドウコンテナ。</b><br>
...
[Releases](https://github.com/lutelute/TerminalIN/releases) から最新版を取得:

- **macOS**: `*-arm64-mac.zip` を解凍し `TiN.app` を `/Applications` へ → 初回起動時に **Accessibility 権限**（システム設定 > プライバシーとセキュリティ > アクセシビリティ）を許可
- **Windows**: `TiN-Setup-*.exe`（NSIS インストーラ）を実行、または `.zip` を展開して `TiN.exe` を起動。未署名のため SmartScreen が出たら「詳細情報」→「実行」
...
TiN は現在 **作者の環境（Apple Silicon Mac / macOS 26 / Terminal.app 中心）でしか動作確認できていません**。
以下の環境での動作報告を募集しています:

| 環境 | 状況 |
|------|------|
| Apple Silicon (M1〜M4) + macOS 26 | ✅ 作者環境で確認済み |
| Apple Silicon + macOS 15 (Sequoia) | ❓ 未確認 — 報告求む |
| Intel Mac | ❓ 未確認 — 報告求む（ビルドターゲットは arm64 のみのため `package.json` の調整が必要かも） |
