...
- **白／黒テーマ** — Crystalline UI をヘッダーまたは設定から即時切り替え
- **コマンドパレット** — `⌘K` / `Ctrl+K` でウィンドウ移動・レイアウト操作を横断検索
...
- Node.js 20 以上
- macOS: Xcode Command Line Tools（`xcode-select --install`）— native addon (ax-helper) のビルドに必要
- Windows: **コンパイラ不要**。ウィンドウ操作は koffi (FFI)、内蔵端末は node-pty で、どちらも prebuilt を同梱します（Visual Studio / node-gyp は不要）
...
# Windows (コンパイル不要):
npm run dist:win         # dist/ に NSIS インストーラ(.exe) / zip を生成 (x64 / arm64)
```
...
| Windows 11 arm64 | 確認済み（実機: 起動・**透明グリッド**・外部ウィンドウ snap / リサイズ・内蔵端末・AI 自動 snap・DPI 200%） |
| Windows 10 / 11 x64 | CI ビルドのみ・実機未確認 |
| Windows の外部ウィンドウ snap | 確認済み（透明グリッドにスロット越しで実ウィンドウが見える・DPI 追従） |
| Windows の仮想デスクトップ移動 | `VirtualDesktopAccessor.dll` を置けば有効（best-effort・opt-in） |
| iTerm2 / Warp / Alacritty の snap | 部分的に確認 |
| マルチディスプレイ構成 | 2 枚まで確認（ワークスペースのディスプレイ移動: Ctrl/Cmd+Shift+M） |
...
### Windows の仮想デスクトップ（任意）

Windows では仮想デスクトップ間の一括移動（◀ ▶ ↗）は **opt-in** です。外部ウィンドウのデスクトップ移動には
非公開 API が必要なため、それを安定ラップした第三者 DLL [VirtualDesktopAccessor.dll](https://github.com/Ciantic/VirtualDesktopAccessor)
を次のいずれかに置くと有効になります（DLL は同梱していません）:

- 環境変数 `TIN_VDA_DLL` に DLL のフルパスを指定
- `%APPDATA%\TiN\VirtualDesktopAccessor.dll` に配置
- `TiN.exe` と同じフォルダに配置

DLL をロードできると Space ボタン（◀ ▶ ↗）が自動表示され、TiN・組み込み端末・スナップ済み外部ウィンドウを
まとめて別デスクトップへ移動できます。DLL が無い／その Windows ビルドで動かない場合は自動的に無効化され、
Space ボタンは表示されません（他の機能はすべて使えます）。

> 注: VirtualDesktopAccessor.dll は Windows のビルドに依存するため、Windows Update で動かなくなる可能性があります。
...
```
▦  ワークスペース名  │ タブ1  タブ2  タブ3 │ +  Fill  3×2  Edit  Retile │ ⌘K  Theme  View
```

| 操作 | 機能 |
|--------|------|
| **左端のGridアイコン** | Workspace Inspectorを開く／閉じる |
| **Viewアイコン** | グリッドモード ⇔ タブモード 切り替え |
| **タブ** | snapped アプリのタブ。クリックでそのアプリを前面化 |
| **×** | タブの × でそのアプリを unsnap |
| **+** | 新しい組み込み PTY ターミナルを追加 |
| **3×2** | グリッドサイズを変更（クリックでメニュー、スクロールで順送り） |
| **Edit** | グリッド分割比率を手動調整（ドラッグで各セルのサイズ変更） |
| **Retile** | 全 snapped アプリを現在の Space に集めてグリッドに再配置 |
| **⌘K / Ctrl+K** | コマンドパレット。Flip、Space移動、テーマ、設定、各ウィンドウへの移動を検索 |
| **Theme** | Black / Whiteテーマを即時切り替え（設定にも同じ項目あり） |
...
1. 左端の **Gridアイコン** をクリックしてWorkspace Inspectorを開く
...
`⌘K` / `Ctrl+K` を開き、`Move workspace to next Space` または `Move workspace to previous Space` を実行すると、TiN と全 snapped アプリが前後のデスクトップに移動します。
...
`⌘K` / `Ctrl+K` で目的の `Move workspace to Desktop …` を検索すると、指定したデスクトップへ直接移動できます。
...
左端のGridアイコンで開くWorkspace Inspectorは、状態サマリーと各管理セクションで構成されています。
...
├── native/ax-helper.mm  — macOS バックエンド (N-API native addon)
│   ├── AXUIElement 操作（位置・サイズ・raise）
│   ├── CGS API（Space 情報・sticky）
│   └── CGWindowList（layer==0 の全アプリウィンドウを列挙）
└── win-helper.js        — Windows バックエンド (koffi FFI・ビルド不要)
    ├── EnumWindows / SetWindowPos（DWM 不可視縁を補正・DPI 変換）
    ├── BringWindowToTop + AttachThreadInput（raise）
    └── 仮想デスクトップ: VirtualDesktopAccessor.dll があれば有効 (opt-in)
```

両 OS のバックエンドは**同じ API 名・戻り値スキーマ**を持つため、main.js は OS を意識せず同一コードパスで動きます（macOS = ax_helper.node / Windows = win-helper.js)。**daemon バイナリなし** — ウィンドウ操作はすべて TiN 本体プロセス内で実行します（macOS は CDHash 変動による TCC 権限消失問題も回避）。
