...
- macOS 15 (Sequoia) 以降
- Node.js 20 以上（ビルドに使用）
- Xcode Command Line Tools（`xcode-select --install` — native addon のビルドに必要）
- **Accessibility 権限** — 初回起動時に促されます。システム設定 > プライバシーとセキュリティ > アクセシビリティ に TiN を追加

### ソースからビルド & インストール（現在はこの方法のみ）
...
> 📦 ビルド済みバイナリの配布（GitHub Releases）は署名・公証の整備後に予定しています。
