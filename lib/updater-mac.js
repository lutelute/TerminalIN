...
// zip を展開して /Applications/<name>.app を差し替える。
...
function installFromZip(zipPath, updatesDir, destApp) {
  ...
  try {
    ...
    if (!appFile) return { ok: false, error: 'zip の中に .app が見つかりません' };

    const sourceApp = path.join(extractDir, appFile);
    if (fs.existsSync(destApp)) execSync(`rm -rf "${destApp}"`);
    execSync(`ditto "${sourceApp}" "${destApp}"`, { timeout: 120000 });
    // 未署名アプリの Gatekeeper 対策。属性が無いと xattr は非0で返るので握りつぶす。
    try { execSync(`xattr -rd com.apple.quarantine "${destApp}" 2>/dev/null`); } catch {}
...
module.exports = { initMacAutoUpdate, isNewer, selectMacAsset, evaluateRelease, appRootFromExecPath };
