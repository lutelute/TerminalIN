// lib/updater.js — 自動アップデートの入口
//
// Windows: electron-updater + GitHub Releases (このファイル)
// macOS  : 未署名で Squirrel.Mac に乗れないため lib/updater-mac.js (ditto+xattr) へ委譲
//
...
//
// 安全策:
//  - このファイルの経路はパッケージ版 (app.isPackaged) かつ Windows のみ。dev では no-op。
...
function initAutoUpdate(getWin) {
  // mac は署名が無く electron-updater(Squirrel.Mac) に乗れないので、GitHub Releases の
  // zip を ditto で差し替える別実装へ委譲する。以下の Windows 経路には手を入れない。
  if (process.platform === 'darwin') {
    try { require('./updater-mac').initMacAutoUpdate(getWin); }
    catch (e) { console.warn('[tin] mac updater 読み込み失敗 — 自動更新は無効:', e && e.message); }
    return;
  }

  // dev 実行や Windows 以外は対象外。
