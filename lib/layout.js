...
//
// 候補は cols=1..maxCols の各列数に対し rows=ceil(need/cols) の最小行数のみ
// (行を余分に足しても空きセルが増えるだけで選ぶ理由がない)。
// スコア = 空きセル数 * EMPTY_WEIGHT + |ln(セル縦横比)| の最小を採る:
//   - 空きセル = 「窓数と一致させる」という目的からの逸脱なのでペナルティ
//   - セル縦横比 (セル幅/セル高) は 1 (正方形) に近いほど端末窓として使いやすい。
//     比の遠さは対数で測る (1/2 と 2 を同距離にするため — 線形だと縦長が過剰に有利)
//   - EMPTY_WEIGHT=0.6 は「need=1 で 1x1 が勝つ」「need=5 で 5x1 より 3x2(空き1) が
//     勝つ」の両方を満たす値。変えるときはテストの形状表で確認すること
// area 不明時は 16:9 (横長ディスプレイ) と仮定する。
// どの候補も maxRows に収まらないほど need が大きい場合は上限形状を返す
// (収まらない分の扱い = skipped は呼び出し側の責務)。
function fitGridDims(need, { maxCols = 20, maxRows = 20, areaWidth, areaHeight } = {}) {
  if (!Number.isFinite(need) || need < 1) return { cols: 1, rows: 1 };
  const hasArea = areaWidth > 0 && areaHeight > 0;
  const aw = hasArea ? areaWidth : 16;
  const ah = hasArea ? areaHeight : 9;
  const EMPTY_WEIGHT = 0.6;
  let best = null;
  for (let cols = 1; cols <= maxCols; cols++) {
    const rows = Math.ceil(need / cols);
    if (rows > maxRows) continue;
    const cellAspect = (aw / cols) / (ah / rows);
    const score = (cols * rows - need) * EMPTY_WEIGHT + Math.abs(Math.log(cellAspect));
    if (!best || score < best.score) best = { cols, rows, score };
  }
  return best ? { cols: best.cols, rows: best.rows } : { cols: maxCols, rows: maxRows };
}
